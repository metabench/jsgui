...
// Could be a type of web resource.
//  Except publisher is about publishing a non-web resource

// Want to make this handle websockets too.
//  Seems easier than making separate classes for websockets and page responses. The two go together.
//  There will be a subscribe / observe method that will use websockets / socksjs. The get/set will use HTTP requests.
...
define(['../../web/jsgui-html', 'os', 'http', 'url', './web', '../../web/jsgui-je-suis-xml', 'cookies', './publisher'], 

	function(jsgui, os, http, libUrl, Web_Resource, JeSuisXML, Cookies, Resource_Publisher) {
	...
	var Resource_Pool_Publisher = Resource_Publisher.extend({
		...
		'init': function(spec) {
			...
			if (spec.resource) this.set('resource', spec.resource);
	...
	return Resource_Pool_Publisher;
